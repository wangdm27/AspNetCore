...
namespace AspNetCore.RabbitMq
{
    ...
    /// 发布者池开启发布确认并绑定 ChannelConfirmTracker；消费者池（enableConfirms=false）建普通通道，免去 confirm 开销。
    /// </remarks>
    internal sealed class RabbitMqChannelPool : IRabbitMqChannelPool, IRabbitMqChannelPoolLease
    {
        private readonly IRabbitMqConnection _connection;
        private readonly bool _enableConfirms;
        ...
        /// <param name="enableConfirms">是否开启发布确认并绑定 ChannelConfirmTracker；消费者池传 false。</param>
        public RabbitMqChannelPool(IRabbitMqConnection connection, int poolSize, bool enableConfirms = true)
        {
            _connection = connection;
            _enableConfirms = enableConfirms;
        ...
        public async ValueTask<PooledChannelLease> RentAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                while (_pool.TryDequeue(out var entry))
                {
                    ...
                    if (entry.Tracker is { } staleTracker)
                    {
                        await staleTracker.DisposeAsync();
                    }
                ...
                ChannelConfirmTracker? tracker = null;
                IChannel channel;
                if (_enableConfirms)
                {
                    ...
                    channel = await conn.CreateChannelAsync(options, cancellationToken);
                    tracker = new ChannelConfirmTracker(channel);
                }
                else
                {
                    // 消费者池不发布，无需 confirm，免去 broker confirm 队列与客户端 tracking 开销
